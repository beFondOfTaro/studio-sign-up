function initMyTable(tableConfig) {
    ...
    $.extend(defaultConfig,tableConfig);
    //初始化表头
    initThead(defaultConfig);
    //生成表身体的内容
    rendPageData(defaultConfig);
}

function initThead(defaultConfig) {
    var tableElem = $(defaultConfig.tableId);
    //初始化表头
    tableElem.html('<thead></thead><tbody></tbody>');
    for (var cols of defaultConfig.cols){
        //生成表头的行
        var theadElem = $(defaultConfig.tableId+' thead');
        var theadHtml = theadElem.html() + "<tr></tr>";
        theadElem.html(theadHtml);
        //生成表头行内容
        var thead_trElem = $(defaultConfig.tableId+' thead tr');
        ...
        //生成操作按钮列的表头
        if (defaultConfig.actionCol){
            thead_trElem.html(thead_trElem.html()+'<th>操作</th>');
        }
    }
}

function rendPageData(defaultConfig) {
    for (var cols of defaultConfig.cols){
        $.ajax({
            type: defaultConfig.method,
            url: defaultConfig.url,
            data: defaultConfig.request,
            dataType: "json",
            success: function (data) {
                //生成翻页栏
                rendPagination(defaultConfig,data);
                //生成表身体的内容
                rendTbody(defaultConfig,cols,data);
            }
        });
    }
}
...
//生成表身体的内容
function rendTbody(defaultConfig,cols,data) {
    var rendActionButton = function (Elem) {
        Elem.innerHTML = Elem.innerHTML + '<td><button type="button" class="btn btn-success">修改</button>'+
            '<button type="button" class="btn btn-danger">删除</button></td>';
    };
    var count = 1;
    for (var col of cols){
        var tbodyHtml = $(defaultConfig.tableId+' tbody').html();
        var tbody_trElem;
        var tbody_trHtml;
        for (var i = 0;i<data.data[defaultConfig.response.dataName].length;i++) {
            //只有在第一次生成行的时候才写第一列的内容
            if (count === 1) {
                //生成表身体内容
                tbodyHtml = $(defaultConfig.tableId + ' tbody').html() + '<tr></tr>';
                $(defaultConfig.tableId + ' tbody').html(tbodyHtml);
                tbody_trElem = $(defaultConfig.tableId + ' tbody tr')[i];//第i行的tr元素
                //第一列的内容，即序号
                tbody_trHtml = '<th>' + ((defaultConfig.request.page-1)*defaultConfig.request.size+i+1) + '</th>';
                tbody_trElem.innerHTML = tbody_trHtml;
            }
            //生成当前列的所有数据
            tbody_trElem = $(defaultConfig.tableId + ' tbody tr')[i];//第i行的tr元素
            tbody_trHtml = tbody_trElem.innerHTML;
            tbody_trHtml = tbody_trHtml + '<td>' + data.data[defaultConfig.response.dataName][i][col.field] + '</td>';
            tbody_trElem.innerHTML = tbody_trHtml;
            //生成操作按钮
            if (defaultConfig.actionCol && count === cols.length){
                rendActionButton($(defaultConfig.tableId + ' tbody tr')[i]);
            }
        }
        count++;
    }
...
/**生成翻页栏
 *
 * @param defaultConfig 表格配置的json
 * @param data  服务端返回的数据
 */
function rendPagination(defaultConfig,data) {
    //在beforeElem元素之后初始化翻页组件
    var navAriaLabel = defaultConfig.pageNav.navAriaLabel;
    var beforeElem = $(defaultConfig.pageNav.beforeElem);
    ...
    //生成翻页组件按钮
    var pageNum = defaultConfig.pageNav.buttonNum;//显示翻页按钮数量
    var totalCounts = data.data[defaultConfig.response.countName];
    var totalPages = data.data[defaultConfig.response.totalPages];//总的页数
    var nav_ulElem = $(defaultConfig.pageNav.beforeElem).nextAll("[aria-label='"+navAriaLabel+"']").find('ul');//列表元素
    ...
    //显示页码的第一个按钮
    var firstPage = Math.floor((defaultConfig.request.page-1) /pageNum)*pageNum + 1;
    //如果要显示的页码按钮未超过设定的数量，则显示全部按钮
    if ((totalPages-firstPage) < pageNum){
        for (var i = firstPage; i <= totalPages;i++){
            //第i页的按钮
            //如果为当前页，样式为active
            if (defaultConfig.request.page === i){
            ...
            if (i === firstPage){
                //如果当前请求的页码为1，上一页不可点击
                if (defaultConfig.request.page === 1){
            ...
            if (i === totalPages){
                //如果总页数等于当前页，下一页不可点击
                if (totalPages === defaultConfig.request.page){
    ...
    //如果总页数超过设定的数量，隐藏部分按钮
        ...
        for (var i = 0; i < pageNum;i++){
            ...
            //第i页按钮
            if (defaultConfig.request.page === currentPage){
...
function turnPage(defaultConfig,currentElem) {
    var ariaLabel = currentElem.getAttribute('aria-label');
    //如果为上一页的按钮
    if (ariaLabel === 'Previous'){
        defaultConfig.request.page--;
        initMyTable(defaultConfig);
    }
    //如果为下一页的按钮
    ...
    //带页码的按钮
        ...
        defaultConfig.request.page = parseInt(currentElem.text);//当前页码
        initMyTable(defaultConfig);
    }
}